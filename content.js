...
window.addEventListener("load",()=>{
    setTimeout(()=>{
        let newHelpRequest=document.querySelector(".chr-doubt-item-v2__btn--accept-request");
        if(newHelpRequest){
            ...
            chrome.storage.sync.get("acceptRequestKey",(data)=>{
                if(data.acceptRequestKey){
                    ...
                    let modal=document.querySelector(".chr-open-request-accept-modal__label");
                    if(modal){
                        //if modal is floated over request , we get the hold of slot option to click it aswell
                        let slotButton=document.querySelector(".chr-open-request-accept-modal__book-slot-btn");
                        if(slotButton){
                            slotButton.click();
                        }
                    }
